        ...
        for (let x = 0; x < 12; x++) {
            ...
            if (qtdCarros > 1600000) {
                if (qtdCarros > 2000000) {
                    mensagens.push(`<label style = "color: #C70039">Maior média já registrada em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label><br>`);
                }
                else {
                    mensagens.push(`<label style = "color: #FF5733">Média de carros muito alta em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label><br>`);
                }
            }
            else {
                if (qtdCarros < 500000) {
                    if (qtdCarros < 150000) {
                        mensagens.push(`<label style = "color: #FF5733">Menor média de carros em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label><br>`);
                    }
                    else {
                        mensagens.push(`<label style = "color: #FFC300">Média de carros muito baixa em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label><br>`);
                    }
                }
            }
        }
