...
function registrar_leitura(pontos, dados, dataRegistro) {
    ...
    console.log(`${dados}`);
     ...
     banco.conectar().then(() => {
		 ...
		 let query = 'INSERT into Registro (QtdCarros, momento, fkPonto) values ';
		 
		 for(var i = 0; i<dados.length; i++){
			 const pontoFinal = i+1==dados.length? ';' : ', ';
            query += `(${dados[i]}, '${dataRegistro}', ${pontos[i]})` + pontoFinal;
		 }
...
const momento_atual = new Date('2020/08/19-16:00:00'); // Mudar pra se adequar a apresentação.
let milisegundos = momento_atual.getTime();


function agora() {
    
    let data_nova = new Date(milisegundos);
    data_nova = new Date(`${data_nova.toLocaleDateString()}-${data_nova.toLocaleTimeString()}-03:00`);
    const retorno = `${data_nova.toLocaleDateString()} ${data_nova.toLocaleTimeString()}`;
...
if (gerar_dados_aleatorios) {
	...
	setInterval(function() {
		...
		var pontos = [4, 5, 6, 7, 8, 9];

        let dados = [];

        for(var contador = 0; contador < pontos.length; contador++){

            dados.push(parseInt(Math.random()*500));
        }

        registrar_leitura(pontos, dados, dataRegistro);
        
    }, intervalo_geracao_aleatoria_segundos * 5000);
