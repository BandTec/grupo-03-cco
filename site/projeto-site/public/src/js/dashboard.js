...
const cores = ['#39b4db', '#679bfa', '#a384f3', '#d864d6', '#ff0066', '#ff1c1c'];
        ...
        if (response.ok) {
            ...
            response.json().then(function (retornoBD) {
                ...
                for (var i = 0; i < rodoviasObtidas.length; i++) {
                    ...
                    divRodovias.innerHTML += `<button class="btnRodovias" onclick=colocarOpcoes(${rodoviasObtidas[i].idRodovia})>${rodoviasObtidas[i].nomeRodovia}</button>`;
                }
            });
        }
    });
    ...
    fetch(`/leituras/pegarTrechos/${idRodovia}`).then(response => {
        ...
        if (response.ok) {
            ...
            response.json().then(function (retornoBD) { //Transformando oq retornou od BD em um json
                ...
                divTrechos.innerHTML += `<div style="padding-top:60px;"><button style="padding: 15px 10px;width:250px;" onclick="mostrarGrafico()">Visualizar</button></div>`;
            }).finally(function () {
        ...
        for (let x = 0; x < 12; x++) {
            ...
            if (qtdCarros > 1600000) {
                if (qtdCarros > 2000000) {
                    mensagens.push(`<li class="avisosRelatorio lRuim">Maior média já registrada em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label></li>`);
                } 
                else {
                    mensagens.push(`<li class="avisosRelatorio lRuim">Média de carros muito alta em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label></li>`);
                }
            }
            else {
                if (qtdCarros < 500000) {
                    if (qtdCarros < 150000) {
                        mensagens.push(`<li class="avisosRelatorio lBom"><label style = "color: #FF5733">Menor média de carros em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</label></li>`);
                    }
                    else {
                        mensagens.push(`<li class="avisosRelatorio lBom"><p>Média de carros muito baixa em ${configMensal.data.labels[x]} no ${configMensal.data.datasets[i].label}</p></li>`);
                    }
                }
            }
        }
...
function alertar(mensagens) {
    divStatusTitulo.innerHTML = '<h3 class="text-escolha">Avisos</h3>';
