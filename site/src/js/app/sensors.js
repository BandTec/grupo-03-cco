...
/*  ESSE É O PEDAÇO DEO NOSSO PROJETO */
function trc5000() {
    //valor minimo que se pode trabalhar
    min = 0;
    //valor maximo que se pode trabalhar
    max = 1;
    //Random recebe um valor randomico  que varia ente minimo e maximo
    let random = Math.random() * (max - min) + min;
    //verifica se random é maior que 0.7 se sim retorna 1 se não retorno 0
    if (random >= 0.7) {
        return 1
    } else if (random <= 0.7) {
        return 0
